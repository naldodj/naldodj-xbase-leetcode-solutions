...
static function myAtoi(cString as character)
    ...
    for i:=i to nStringLen
        ...
        if (nChar>=48).and.(nChar<=57) // Verifica se o caractere é numérico
            nResult*=10
            nResult+=(nChar-48)
        else
            // Se encontrar um caractere não numérico, interrompe
            exit
        endif
    next i
