...
static function BigAdd(cNum1,cNum2)
    ...
    for i:=nMaxLen to 1 step -1
        ...
        nSum:=((n1+n2)+nCarry)
        nCarry:=Int(nSum/10)
        cRes:=(hb_NToC(Mod(nSum,10))+cRes)
    next i

    // Se sobrou carry
    if (nCarry>0)
        cRes:=(hb_NToC(nCarry)+cRes)
    endif
...
static function BigHalve(cNum as character)
    ...
    begin sequence
        ...
        cRes:=if(Empty(BigTrimLeftZeros(cRes)),"0",BigTrimLeftZeros(cRes))
...
static function BigTrimLeftZeros(cNum as character)
    cNum:=LeftCharRemove(cNum,"0",.T.)
    return(cNum)

/* -------------------------------------------------------------
   LeftCharRemove(cNum)
   Remove caracteres à esquerda
   ------------------------------------------------------------- */
static function LeftCharRemove(cStr as character,cChr as character,lAll as logical)
