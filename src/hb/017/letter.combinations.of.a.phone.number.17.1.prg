...
static function LetterCombinations(cDigits)

    local aTemp as array
    local aResult as array

    local hDigitToLetters as hash
    ...
    begin sequence
        ...
        aTemp:=Array(0)
        aResult:=hDigitToLetters[SubStr(cDigits,1,1)]

        for i:=2 to Len(cDigits)
            aSize(aTemp,0)
            for j:=1 to Len(aResult)
                for k:=1 to Len(hDigitToLetters[SubStr(cDigits,i,1)])
                    aAdd(aTemp,aResult[j]+hDigitToLetters[SubStr(cDigits,i,1)][k])
                next k
            next j
            aResult:=aClone(aTemp)
        next i
