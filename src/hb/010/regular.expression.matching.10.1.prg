...
static function IsMatchRecursive(cString as character,cPattern as character,nMin as numeric,nMax as numeric)
    ...
    begin sequence
        ...
        //s contains only lowercase English letters.
        lMatch:=simplePatternMatch(cString,nStrSize,"a-z",3)
        if (!lMatch)
            break
        endif

        //p contains only lowercase English letters, '.', and '*'.
        lMatch:=simplePatternMatch(cPattern,nPatSize,"a-z.*",5)
        if (!lMatch)
            break
        endif
...
static function simplePatternMatch(cString as character,nStrSize as numeric,cPattern as character,nPatSize as numeric)

    local cTmp as character,cWildcardPattern as character:=""

    local nAsc as numeric
    local nMin as numeric:=0
    local nMax as numeric:=0

    local lMatch as logical:=.F.

    local i

    for i:=1 to nPatSize
        cTmp:=subStr(cPattern,i,1)
        if (cTmp$".*")
            if (!cTmp$cWildcardPattern)
                cWildcardPattern+=cTmp
            endif
            loop
        endif
        nAsc:=Asc(cTmp)
    ...
    for i:=1 to nStrSize
        cTmp:=subStr(cString,i,1)
        if (cTmp=="-")
            loop
        endif
        nAsc:=Asc(cTmp)
        lMatch:=((cTmp$cWildcardPattern).or.((nAsc>=nMin).and.(nAsc<=nMax)))
        if (!lMatch)
            exit
        endif
    next i
